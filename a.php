...
// Obsługa przesłania nowego postu
if (isset($_POST['newpostarea'])) {
    $newPostContent = $_POST['newpostarea'];
    ...
    try {
        ...
        $stmt = $pdo->prepare($query);
        $stmt->bindValue(':content', $newPostContent, PDO::PARAM_STR);
        $stmt->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT); // Przykład przypisania user_id z sesji
        $stmt->execute();

        // Inne operacje na bazie danych

        // Zatwierdzenie transakcji
        $pdo->commit();

        // Ustawienie ciasteczka
        setcookie('newPost', $newPostContent, time() + (86400 * 30), "/"); // Ciasteczko ważne przez 30 dni
        ...
        $pdo->rollBack();
        echo "Błąd dodawania postu do bazy danych: " . $e->getMessage();
    }
}
